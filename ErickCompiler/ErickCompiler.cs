...
namespace ErickCompiler
{
    class ErickCompiler
    {
        ...
        public struct WorckSpace
        {
            ...
            {
                if (
                    BuildWith == "EASM_Hlvc" || BuildWith == "EASM"
                    )
                {
                    ...
                    )
                {
                    ...
                    {
                        if (
                            lines[i].Trim() == "import" && lines[i + 2].Trim() == ";"
                            )
                        {
                            //Console.WriteLine(GetTheFirstAparitionOfTheModule(lines[i + 1].Trim()));
                            outpud = outpud.Replace(
                                "import\n" + lines[i + 1] + "\n;",
                                (File.Exists(GetTheFirstAparitionOfTheModule(lines[i + 1].Trim() + ".lima")) ? BuildFile(File.ReadAllText(GetTheFirstAparitionOfTheModule(lines[i + 1].Trim() + ".lima")), BuildWith) : "")
                                );
                        }
                    }
                        ...
                        )
                    {
                        ...
                        while (instruccion != "end")
                        {
                                ...
                                )
                            {
                                ...
                                File.WriteAllText(Path.Join(Uri, NormalizePath(ResultFile)), 
                                    BuildWith == "EASM_Hlvc" ? EasmCompiler.ConvertCode(EasmCompiler.TranslateHightLevelToAsm(le)) : (
                                        BuildWith == "EASM" ? EasmCompiler.ConvertCode(le) : ResultFile
                                    )
                                    );
                            }
                            else if (
